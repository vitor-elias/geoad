...
def train_GUNet(model, X, edge_index, N_epochs, lr, training_loss):
...
def evaluate_model(model, data, labels, edge_index, N_epochs, lr=1e-3, training_loss=torch.nn.MSELoss()):
    ...
    for i in range(data.shape[0]):
        ...
        X = data[i,:,:]
        label = labels[i,:]
        model = train_GUNet(model, X, edge_index, N_epochs, lr, training_loss)
...
def main(args):
    ...
    def objective(trial):
        ...
        model = GraphUNet(n_timestamps, hidden_channels, n_timestamps, depth, pool_ratio)                       
...
if __name__ == "__main__":
    ...
    for n_clusters in [30, 20, 10, 5, 2]:
        ...
        parser.add_argument('--model', type=str, default='GUNet')
        ...
        parser.add_argument('--N_epochs', type=int, nargs='+', default=[50, 100, 250, 200, 500])
