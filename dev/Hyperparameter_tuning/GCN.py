...
def train_GCN(model, X, edge_index, N_epochs, lr, training_loss):
...
def evaluate_model(model, data, labels, edge_index, N_epochs, lr=1e-3, training_loss=torch.nn.MSELoss()):
    ...
    for i in range(data.shape[0]):
        ...
        X = data[i,:,:]
        label = labels[i,:]
        model = train_GCN(model, X, edge_index, N_epochs, lr, training_loss)
...
def main(args):
    ...
    def objective(trial):
        ...
        if args.model=='GCN2MLP':
            model = models.GCN2MLP(n_timestamps, n_layers, reduction)
        if args.model=='GCNAE':
        ...
        auc_list = evaluate_model(model, data, labels, edge_index, N_epochs, lr, training_loss)
    ...
    data, labels, data_dfs, G, nodes = utils.generate_data(df_ds, args.graph_clusters, args.select_cluster, noise=0.1,
...
if __name__ == "__main__":
    ...
    for n_clusters in [30, 20, 10, 5, 2]:
        ...
        parser.add_argument('--model', type=str, default='GCN2MLP')
        ...
        parser.add_argument('--anomalous_nodes', type=int, default=5)
