...
def train_GALA(model, X, edge_index, edge_weight, N_epochs, lr, training_loss):
...
def evaluate_model(model, data, labels, edge_index, edge_weight, N_epochs, lr=1e-3, training_loss=torch.nn.MSELoss()):
    ...
    for i in range(data.shape[0]):
        ...
        X = data[i,:,:]
        label = labels[i,:]
        model = train_GALA(model, X, edge_index, edge_weight, N_epochs, lr, training_loss)
...
def main(args):
    ...
    def objective(trial):
        ...
        use_weight = trial.suggest_categorical('use_weight', args.use_weight)
        ...
        print(f"- Use weights: {use_weight}", flush=True)
        ...
        model = models.GALA(n_timestamps, n_layers, reduction)
        model = model.to(device)

        if model.layer_dims[-1] <= 2:
            raise optuna.TrialPruned()

        edge_weight = edge_attr if use_weight==True else None
...
if __name__ == "__main__":
    ...
    for n_clusters in [30, 20, 10, 5, 2]:
        ...
        parser.add_argument('--model', type=str, default='GALA')
        ...
        parser.add_argument('--N_epochs', type=int, nargs='+', default=[10, 50, 100, 250, 200, 500, 1000])
        ...
        parser.add_argument('--use_weight', type=str, nargs='+', default=['True', 'False'])
